...
const Blog = ({blogs}) => {
  ...
  return (
    ...
    <link rel="icon" href="/assets/img/shreyans1.png" type="image/icon type"/>
    <link
          rel="stylesheet"
    ...
    <div className={styles.blogheader}>
      <div className={styles.innerdivblog}>
        <h2 style={{'fontFamily' : "Source Code Pro, monospace"}} className={styles.headerblogtitle}>Tech. Entrepreneurship. Electronics. Psychology?</h2>
        <p className={styles.headerblogtitlep} style={{'fontFamily' : "Source Code Pro, monospace"}}>That's me. That's <span className='text-[28px]' style={{'fontFamily' : "Square Peg, cursive"}}> Shreyans. </span></p>
            ...
            return (
              <Link key={item.slug.current} href={"/blog/" + item.slug.current}>
                <div className={styles.blog}>
                  ...
                  <span style={{'fontFamily' : "Josefin Sans, sans-serif"}} className={styles.date}>{getDate(item.createdAt)}</span>
                  </div>
                  <h2 className={styles.titleblog} style={{'fontFamily' : "Josefin Sans, sans-serif"}}>{item.title}</h2>
                  <p className={styles.metastring} style={{'fontFamily' : "Josefin Sans, sans-serif"}}>{item.metadesc.substring(0, 150)}...</p>
                </div>
              </Link>
            );
          })}
