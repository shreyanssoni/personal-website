...
const PLATFORMS = [
  {
    ...
    fallbackUrl: (url: string, text: string) =>
...
export default function ShareButton({ signalId, title, compact, dropUp }: ShareButtonProps) {
  ...
  /** Try native share with image file, fall back to URL popup */
  async function handlePlatformShare(platform: typeof PLATFORMS[number]) {
    ...
    if (navigator.share) {
      try {
        ...
        // File not supported — try native share without image
        ...
        // Other error — fall through to URL fallback
      }
    }

    // Desktop fallback: open URL in popup
    const url = platform.fallbackUrl?.(shareUrl, shareText);
    ...
    <div className="relative" ref={menuRef}>
      ...
      {open && (
        ...
        >
          ...
          {/* Platform buttons — native share with image on mobile, URL fallback on desktop */}
          {PLATFORMS.map((platform) => (
            <button
              key={platform.key}
              onClick={() => handlePlatformShare(platform)}
              disabled={busy}
              className={`w-full flex items-center gap-2.5 px-3 py-2 text-left text-stone-600 transition-all ${platform.color} ${busy ? "opacity-50" : ""}`}
            >
          ...
          <button
            onClick={handleCopy}
            className="w-full flex items-center gap-2.5 px-3 py-2 text-left text-stone-600 transition-all hover:bg-stone-100 hover:text-stone-700"
          >
            {copied ? (
              <>
              ...
              <>
                <Link2 size={14} />
                <span className="font-[family-name:var(--font-soft)] text-[13px]">Copy link</span>
              </>
            )}
          </button>
