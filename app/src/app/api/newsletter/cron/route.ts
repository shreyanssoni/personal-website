...
async function step4_enrich_store(today: string) {
  ...
  const issueId = await createIssue({
    ...
    qs_launched_text: meta.quick_scan?.find(q => q.label === "What Launched")?.summary || meta.quick_scan?.[0]?.summary || "",
    qs_shifting_text: meta.quick_scan?.find(q => q.label === "What's Shifting")?.summary || meta.quick_scan?.[1]?.summary || "",
    qs_watch_text: meta.quick_scan?.find(q => q.label === "What to Watch")?.summary || meta.quick_scan?.[2]?.summary || "",
    raw_count: rawCount,
